...
namespace xieite {
	...
	public:
		...
		void cursor_underscore(bool blink = false) noexcept {
			std::print(this->output, "{}", xieite::io_handle::cursor_underscore_code(blink));
		}
		...
		xieite::keyboard read_key() noexcept {
			const xieite::scope_guard _ = xieite::scope_guard([this, block_prev = this->block_enabled] -> void {
				this->blocking(block_prev);
			});
